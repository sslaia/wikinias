...
import 'widgets/niaspedia_special_pages_bottom_app_bar.dart';

class NiaspediaSpecialPagesScreen extends StatefulWidget {
  final String title;

  const NiaspediaSpecialPagesScreen({super.key, required this.title});

  @override
  State<NiaspediaSpecialPagesScreen> createState() =>
      _NiaspediaSpecialPagesScreenState();
}

class _NiaspediaSpecialPagesScreenState
    extends State<NiaspediaSpecialPagesScreen> {
  ...
  void _navigateToNewPage(String pageTitle) {
    Navigator.of(context).push(
      MaterialPageRoute<void>(
        builder: (context) => NiaspediaSpecialPagesScreen(title: pageTitle),
      ),
    );
  }
    ...
    return SafeArea(
      child: Consumer<SettingsProvider>(
        builder: (context, settingsProvider, child) => Scaffold(
          bottomNavigationBar: NiaspediaSpecialPagesBottomAppBar(
            title: widget.title,
          ),
          body: CustomScrollView(
            slivers: [
              SliverAppBar(
                ...
                floating: true,
                expandedHeight: 230,
                flexibleSpace: FlexiblePageHeader(image: settingsProvider.getProjectSpecialPagesImage()),
                actions: [
